/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 12:02:01 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /System/Library/Frameworks/Foundation.framework/Versions/C/Foundation
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <Foundation/Foundation-Structs.h>
#import <Foundation/NSXMLNode.h>

@class NSString, NSArray;

@interface NSXMLElement : NSXMLNode {

	NSString* _name;
	id _attributes;
	id _namespaces;
	NSArray* _children;
	char _childrenHaveMutated;
	char _zeroOrOneAttributes;
	char _zeroOrOneNamespaces;
	unsigned char _padding;
	NSString* _URI;
	long long _prefixIndex;

}

@property (copy) NSArray * attributes; 
@property (copy) NSArray * namespaces; 
+(id)ps_elementWithData:(id)arg1 allowHTML:(char)arg2 ;
+(id)ps_XHTMLElementWithData:(id)arg1 ;
+(id)_nodeFromLibXML2Node:(void*)arg1 ;
+(id)_elementFromTidyDoc:(TidyDoc*)arg1 element:(TidyNode*)arg2 encoding:(unsigned)arg3 elementClass:(Class)arg4 nodeClass:(Class)arg5 ;
-(void)addValue:(id)arg1 forAttribute:(id)arg2 ;
-(id)firstElementForName:(id)arg1 ;
-(id)valueForAttribute:(id)arg1 ;
-(id)ps_namespace;
-(void)ps_removeChildrenWithLocalName:(id)arg1 URI:(id)arg2 ;
-(id)ps_childrenWithNamespace:(id)arg1 ;
-(id)ps_childrenWithoutNamespace:(id)arg1 ;
-(void)ps_removeChildrenWithLocalName:(id)arg1 URI:(id)arg2 attribute:(id)arg3 value:(id)arg4 ;
-(id)ps_addLinkTo:(id)arg1 rel:(id)arg2 type:(id)arg3 ;
-(id)ps_addChildWithName:(id)arg1 stringValue:(id)arg2 ;
-(id)ps_addChildWithName:(id)arg1 ;
-(id)ps_addLinkTo:(id)arg1 rel:(id)arg2 type:(id)arg3 length:(unsigned long long)arg4 ;
-(id)ps_asData;
-(id)ps_asDocumentString;
-(char)ps_hasName:(id)arg1 inURI:(id)arg2 ;
-(id)ps_addChildWithName:(id)arg1 dateValue:(id)arg2 format:(id)arg3 ;
-(void)_removeChildren:(id)arg1 ;
-(void)_setString:(id)arg1 forAttributeName:(id)arg2 ;
-(id)_lastElementForName:(id)arg1 createIfNeeded:(char)arg2 ;
-(void)_setBool:(char)arg1 forAttributeName:(id)arg2 ;
-(id)_attributeForName:(id)arg1 ofAnyElementNamed:(id)arg2 ;
-(id)_attributesDictionary;
-(id)_elementsForName:(id)arg1 identifier:(id)arg2 ;
-(void)_replaceAttributesAsDictionary:(id)arg1 ;
-(id)_trimmedStringValue;
-(char)_hasPureTextContent;
-(char)_hasChildElements;
-(void)_setStringValueAsCDATA:(id)arg1 ;
-(id)childAtIndex:(unsigned long long)arg1 ;
-(void)insertChild:(id)arg1 atIndex:(unsigned long long)arg2 ;
-(void)insertChildren:(id)arg1 atIndex:(unsigned long long)arg2 ;
-(id)prefix;
-(id)localName;
-(id)attributeForName:(id)arg1 ;
-(void)_XMLStringWithOptions:(unsigned long long)arg1 appendingToString:(id)arg2 ;
-(id)initWithKind:(unsigned long long)arg1 options:(unsigned long long)arg2 ;
-(char)_QNamesAreResolved;
-(id)URI;
-(unsigned long long)countOfAttributes;
-(void)setURI:(id)arg1 ;
-(void)_setQNamesAreResolved:(char)arg1 ;
-(void)_bindAncestorNamespaces;
-(void)removeChildAtIndex:(unsigned long long)arg1 ;
-(void)replaceChildAtIndex:(unsigned long long)arg1 withNode:(id)arg2 ;
-(id)_children;
-(unsigned long long)countOfChildren;
-(id)objectInChildrenAtIndex:(unsigned long long)arg1 ;
-(void)replaceObjectInChildrenAtIndex:(unsigned long long)arg1 withObject:(id)arg2 ;
-(id)canonicalXMLStringPreservingComments:(char)arg1 ;
-(id)XPath;
-(id)namespaceForPrefix:(id)arg1 ;
-(void)addNamespace:(id)arg1 ;
-(void)removeNamespaceForPrefix:(id)arg1 ;
-(id)_canonicalXMLStringPreservingComments:(char)arg1 namespaceString:(id)arg2 relationships:(id)arg3 ;
-(id)attributeForLocalName:(id)arg1 URI:(id)arg2 ;
-(xmlNode*)_libxml2TreeRepresentationWithNamespaces:(id)arg1 ;
-(void)setNamespaces:(NSArray *)arg1 ;
-(id)initWithName:(id)arg1 stringValue:(id)arg2 ;
-(id)initWithName:(id)arg1 URI:(id)arg2 ;
-(id)_initWithName:(id)arg1 URI:(id)arg2 prefixIndex:(long long)arg3 ;
-(void)_resolveName;
-(id)resolveNamespaceForName:(id)arg1 ;
-(id)elementsForLocalName:(id)arg1 URI:(id)arg2 ;
-(long long)_prefixIndex;
-(char)_nameIsEqualToNameOfNode:(id)arg1 ;
-(void)addAttribute:(id)arg1 ;
-(void)insertObject:(id)arg1 inAttributesAtIndex:(unsigned long long)arg2 ;
-(char)_bindNamespaceName:(id)arg1 URI:(id)arg2 ;
-(unsigned long long)countOfNamespaces;
-(void)insertObject:(id)arg1 inNamespacesAtIndex:(unsigned long long)arg2 ;
-(id)objectInNamespacesAtIndex:(unsigned long long)arg1 ;
-(id)_resolveNamespaceForPrefix:(id)arg1 ;
-(char)_setPrefix:(id)arg1 ;
-(id)objectInAttributesAtIndex:(unsigned long long)arg1 ;
-(void)_changeQNamePrefix:(id)arg1 toPrefix:(id)arg2 forURI:(id)arg3 ;
-(void)_changeQNameURI:(id)arg1 toURI:(id)arg2 forPrefix:(id)arg3 ;
-(void)normalizeAdjacentTextNodesPreservingCDATA:(char)arg1 ;
-(void)_addTrustedAttribute:(id)arg1 atIndex:(unsigned long long)arg2 ;
-(id)initWithLocalName:(id)arg1 URI:(id)arg2 ;
-(id)initWithXMLString:(id)arg1 error:(id*)arg2 ;
-(char)validateName:(id*)arg1 error:(id*)arg2 ;
-(void)_setLocalName:(id)arg1 ;
-(id)elementsForName:(id)arg1 ;
-(void)setAttributesWithDictionary:(id)arg1 ;
-(void)setAttributesAsDictionary:(id)arg1 ;
-(void)removeAttributeForName:(id)arg1 ;
-(void)removeObjectFromAttributesAtIndex:(unsigned long long)arg1 ;
-(void)removeObjectFromNamespacesAtIndex:(unsigned long long)arg1 ;
-(NSArray *)namespaces;
-(id)_namespaceForURI:(id)arg1 ;
-(id)resolvePrefixForNamespaceURI:(id)arg1 ;
-(id)init;
-(void)dealloc;
-(void)setName:(id)arg1 ;
-(char)isEqual:(id)arg1 ;
-(unsigned long long)hash;
-(id)name;
-(id)copyWithZone:(NSZone*)arg1 ;
-(id)stringValue;
-(id)objectValue;
-(void)setObjectValue:(id)arg1 ;
-(id)initWithName:(id)arg1 ;
-(NSArray *)attributes;
-(id)children;
-(void)addChild:(id)arg1 ;
-(void)setChildren:(id)arg1 ;
-(void)insertObject:(id)arg1 inChildrenAtIndex:(unsigned long long)arg2 ;
-(void)removeObjectFromChildrenAtIndex:(unsigned long long)arg1 ;
-(void)setAttributes:(NSArray *)arg1 ;
-(unsigned long long)childCount;
@end


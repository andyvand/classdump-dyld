/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 7:58:05 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Users/joris/Desktop/SysDecrypts/64-Bit/Dock
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <Dock/Dock-Structs.h>
#import <Dock/Tile.h>
#import <Dock/DOCKStackActionHandling.h>
#import <Dock/DOCKStackExpandedDataSource.h>
#import <Dock/DOCKStackCollapsedDataSource.h>

@class DOCKStack, NSMutableArray, NSArray, NSString;

@interface DOCKSmartFolderTile : Tile <DOCKStackActionHandling, DOCKStackExpandedDataSource, DOCKStackCollapsedDataSource> {

	DOCKStack* _stack;
	NSMutableArray* _items;
	id _query;
	CFDictionaryRef _itemMap;
	unsigned char _viewAs;
	int _arrangement;
	int _preferredItemSize;
	int _displayAs;
	NSArray* _extraSortAttributes;
	NSArray* _scope;
	NSArray* _resolvedScope;
	NSString* _filelocation;
	NSArray* _sortedTypeAheadArray;
	NSString* _queryAttr;
	NSMutableArray* _blocksToPerformOnStackClose;
	unsigned _finderFilesOnly : 1;
	unsigned _userFilesOnly : 1;

}

@property (assign,nonatomic) unsigned char viewAs;                              //@synthesize viewAs=_viewAs - In the implementation block
@property (assign,nonatomic) int arrangement;                                   //@synthesize arrangement=_arrangement - In the implementation block
@property (assign,nonatomic) int preferredItemSize;                             //@synthesize preferredItemSize=_preferredItemSize - In the implementation block
@property (assign,nonatomic) int displayAs;                                     //@synthesize displayAs=_displayAs - In the implementation block
@property (nonatomic,readonly) NSArray * sortAttributes; 
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (nonatomic,readonly) const char* messageTracerStackType; 
-(char)doAction:(unsigned)arg1 fromKeyboard:(char)arg2 ;
-(OpaqueMenuRefRef)createMenu:(/*function pointer*/void*)arg1 options:(int)arg2 ;
-(char)showMenu:(char)arg1 options:(int)arg2 ;
-(BOOL)allowsForcedSpringing;
-(void)spring:(unsigned)arg1 ;
-(id)copyTilePlist;
-(void)collapseStack;
-(int)preferredItemSize;
-(void)setPreferredItemSize:(int)arg1 ;
-(id)initWithPlist:(id)arg1 ;
-(void)setViewAs:(unsigned char)arg1 ;
-(void)setDisplayAs:(int)arg1 ;
-(id)initWithCannedFSRef:(const FSRef*)arg1 ;
-(const char*)messageTracerStackType;
-(char)performStackActionAtIndex:(unsigned long long)arg1 withEvent:(id)arg2 sender:(id)arg3 ;
-(void)beginStackTypeAhead:(id)arg1 ;
-(unsigned long long)indexForStackTypeAhead:(id)arg1 ;
-(void)endStackTypeAhead;
-(void)stackDidClose;
-(void)showStackAsMenu:(char)arg1 ;
-(unsigned long long)numberOfExpandedStackItems;
-(id)expandedStackValueForKey:(id)arg1 atIndex:(unsigned long long)arg2 withItemInfo:(id)arg3 maximumViewableItems:(unsigned long long)arg4 ;
-(id)expandedStackValueForKey:(id)arg1 withInfo:(id)arg2 ;
-(unsigned long long)numberOfCollapsedStackItems;
-(id)collapsedStackValueForKey:(id)arg1 atIndex:(unsigned long long)arg2 ;
-(unsigned char)viewAs;
-(int)displayAs;
-(char)_createQuery:(id)arg1 ;
-(id)_initWithFSRef:(const FSRef*)arg1 containerRef:(const FSRef*)arg2 ;
-(void)_releaseQuery;
-(char)_createQuery:(id)arg1 searchScopes:(id)arg2 ;
-(NSArray *)sortAttributes;
-(void)_queryFinish:(id)arg1 ;
-(void)_queryUpdate:(id)arg1 ;
-(void)_addStackCloseBlock:(/*^block*/id)arg1 ;
-(id)_createFunction:(id)arg1 ;
-(int)arrangement;
-(void)setArrangement:(int)arg1 ;
-(id)initWithFSRef:(const FSRef*)arg1 ;
-(void)doCommand:(unsigned)arg1 ;
-(id)stack;
-(void)dealloc;
-(void)render;
-(id)typeString;
@end


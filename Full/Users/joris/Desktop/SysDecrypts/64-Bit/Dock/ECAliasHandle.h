/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 7:58:05 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Users/joris/Desktop/SysDecrypts/64-Bit/Dock
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/


#import <Dock/Dock-Structs.h>
@interface ECAliasHandle : NSObject {

	AliasRecord* _alias;
	char _changed;

}

@property (assign,nonatomic) AliasRecord* alias;              //@synthesize alias=_alias - In the implementation block
-(id)initWithAliasHandle:(AliasRecord*)arg1 ;
-(id)initWithPtr:(const void*)arg1 size:(long long)arg2 ;
-(void)setAlias:(AliasRecord*)arg1 ;
-(AliasRecord*)alias;
-(void)dealloc;
-(id)resolve:(char*)arg1 ;
@end


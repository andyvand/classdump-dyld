/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 10:35:24 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Applications/Utilities/AirPort Utility.app/Contents/MacOS/AirPort Utility
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <AirPort Utility/StepByStepNSViewController.h>

@class NSView, NSPopUpButton, NetworkAndDeviceNameNSViewController;

@interface StepByStepNSViewController_ExtendWired_Config : StepByStepNSViewController {

	NSView* selectionView;
	NSPopUpButton* baseSelectionPopup;
	NetworkAndDeviceNameNSViewController* networkAndDeviceNameView;

}

@property (assign) NSView * selectionView; 
@property (assign) NSPopUpButton * baseSelectionPopup; 
-(id)outResultsDict;
-(void)handleBasePopupChanged:(id)arg1 ;
-(void)setSelectionView:(NSView *)arg1 ;
-(NSPopUpButton *)baseSelectionPopup;
-(void)setBaseSelectionPopup:(NSPopUpButton *)arg1 ;
-(void)dealloc;
-(void)awakeFromNib;
-(NSView *)selectionView;
@end


/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:11:03 AM Central European Standard Time
* Operating System: Versie 10.10.2 (build 14C109)
* Image Source: /Applications/HWMonitor.app/Contents/MacOS/HWMonitor
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <AppKit/NSToolbarItem.h>

@class NSView;

@interface PrefsToolbarItem : NSToolbarItem {

	NSView* _linkedView;

}

@property (nonatomic,retain) NSView * linkedView;              //@synthesize linkedView=_linkedView - In the implementation block
-(NSView *)linkedView;
-(void)setLinkedView:(NSView *)arg1 ;
@end


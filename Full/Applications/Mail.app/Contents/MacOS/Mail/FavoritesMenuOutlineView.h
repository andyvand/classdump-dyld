/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:37:27 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Applications/Mail.app/Contents/MacOS/Mail
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <Mail/Mail-Structs.h>
#import <AppKit/NSOutlineView.h>

@class FavoritesMenuController, NSTrackingArea;

@interface FavoritesMenuOutlineView : NSOutlineView {

	FavoritesMenuController* _controller;
	NSTrackingArea* _trackingArea;

}

@property (assign,nonatomic,__weak) FavoritesMenuController * controller;              //@synthesize controller=_controller - In the implementation block
@property (nonatomic,retain) NSTrackingArea * trackingArea;                            //@synthesize trackingArea=_trackingArea - In the implementation block
-(void)_favoritesMenuOutlineViewCommonInitWithFrame:(CGRect)arg1 ;
-(void)setTrackingArea:(NSTrackingArea *)arg1 ;
-(void)dealloc;
-(id)initWithCoder:(id)arg1 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(FavoritesMenuController *)controller;
-(NSTrackingArea *)trackingArea;
-(void)scrollWheel:(id)arg1 ;
-(void)mouseDown:(id)arg1 ;
-(void)mouseExited:(id)arg1 ;
-(void)setController:(FavoritesMenuController *)arg1 ;
-(void)updateTrackingAreas;
-(void)mouseMoved:(id)arg1 ;
@end


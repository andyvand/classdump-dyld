/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:37:28 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Applications/Mail.app/Contents/MacOS/Mail
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <Mail/TemporaryAttachmentOpener.h>

@interface InvitationAttachmentOpener : TemporaryAttachmentOpener {

	char _openImmediately;

}

@property (nonatomic,readonly) char openImmediately;              //@synthesize openImmediately=_openImmediately - In the implementation block
-(id)initWithAttachments:(id)arg1 openImmediately:(char)arg2 ;
-(void)beginSaveOfAttachments:(id)arg1 toTemporaryFolderWithName:(id)arg2 taskName:(id)arg3 ;
-(char)openImmediately;
-(void)_saveAttachmentsToICalCaches:(id)arg1 ;
@end


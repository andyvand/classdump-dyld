/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:37:28 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Applications/Mail.app/Contents/MacOS/Mail
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <Mail/Mail-Structs.h>
#import <AppKit/NSButton.h>

@interface MailBarContainerButton : NSButton {

	char _isPerformingSearchScope;

}

@property (assign,nonatomic) char isPerformingSearchScope; 
-(char)isPerformingSearchScope;
-(void)setIsPerformingSearchScope:(char)arg1 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(char)shouldSetFontSmoothingBackgroundColor;
@end


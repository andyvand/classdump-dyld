/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:39:42 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Applications/Maps.app/Contents/MacOS/Maps
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <Maps/Maps-Structs.h>
#import <AppKit/NSTextField.h>

@class NVMultilineCheckboxButton;

@interface NVMultilineCheckboxLabel : NSTextField {

	NVMultilineCheckboxButton* _checkbox;

}

@property (nonatomic,retain) NVMultilineCheckboxButton * checkbox;              //@synthesize checkbox=_checkbox - In the implementation block
-(NVMultilineCheckboxButton *)checkbox;
-(void)setCheckbox:(NVMultilineCheckboxButton *)arg1 ;
-(void)setStringValue:(id)arg1 ;
-(id)hitTest:(CGPoint)arg1 ;
@end


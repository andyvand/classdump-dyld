/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:39:43 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Applications/Maps.app/Contents/MacOS/Maps
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/


#import <Maps/Maps-Structs.h>
@class NSString, GEOMapServiceTraits;

@interface NVSearchManagerContext : NSObject {

	char _allowLocationFix;
	int _source;
	NSString* suggestionsPrefix;
	GEOMapServiceTraits* _traits;

}

@property (assign) int source;                                            //@synthesize source=_source - In the implementation block
@property (nonatomic,retain) NSString * suggestionsPrefix; 
@property (assign) char allowLocationFix;                                 //@synthesize allowLocationFix=_allowLocationFix - In the implementation block
@property (nonatomic,readonly) GEOMapServiceTraits * traits;              //@synthesize traits=_traits - In the implementation block
-(void)setAllowLocationFix:(char)arg1 ;
-(id)initWithTraits:(id)arg1 source:(int)arg2 allowLocationFix:(char)arg3 ;
-(char)allowLocationFix;
-(void)setMapRect:(SCD_Struct_NV6)arg1 ;
-(void)setSuggestionsPrefix:(NSString *)arg1 ;
-(NSString *)suggestionsPrefix;
-(GEOMapServiceTraits *)traits;
-(int)source;
-(void)setSource:(int)arg1 ;
@end


/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:08:28 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Library/Application Support/iLifeMediaBrowser/Plug-Ins/iLMBiPhotoPlugin2012.ilmbplugin/Contents/MacOS/iLMBiPhotoPlugin2012
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/


#import <iLMBiPhotoPlugin2012/iLMBiPhotoPlugin2012-Structs.h>
@class IP2012_RKDatabase, NSString, ILMB_IPHOTO_2012_HgLockedArray, ILMB_IPHOTO_2012_HgLockedDictionary, ILMB_IPHOTO_2012_HgLockedSet, ILMB_IPHOTO_2012_HgRecentChange;

@interface IP2012_RKCustomSortOrder : NSObject {

	int _spinlock;
	IP2012_RKDatabase* m_database;
	NSString* m_containerUuid;
	NSString* m_purpose;
	ILMB_IPHOTO_2012_HgLockedArray* m_customEntries;
	ILMB_IPHOTO_2012_HgLockedDictionary* m_uuidToCustomEntry;
	ILMB_IPHOTO_2012_HgLockedSet* m_badCustomEntries;
	ILMB_IPHOTO_2012_HgRecentChange* m_recentChange;
	CFArrayRef m_observers;
	long long m_notifyLevel;

}

@property (nonatomic,retain) ILMB_IPHOTO_2012_HgRecentChange * recentChange; 
+(id)containersWithCustomOrdersContainingAnyOfUuids:(id)arg1 fromDatabase:(id)arg2 ;
+(id)purposesForContainerUuid:(id)arg1 fromDatabase:(id)arg2 ;
+(id)loadCustomEntriesForContainerUuids:(id)arg1 purpose:(id)arg2 fromDatabase:(id)arg3 ;
-(id)customOrderArray;
-(void)setRecentChange:(ILMB_IPHOTO_2012_HgRecentChange *)arg1 ;
-(ILMB_IPHOTO_2012_HgRecentChange *)recentChange;
-(void)notifyOfChangeForKeyPath:(id)arg1 ;
-(id)sortVersionUuids:(id)arg1 forAlbum:(id)arg2 ;
-(id)sortItems:(id)arg1 ascending:(char)arg2 uuidSelector:(SEL)arg3 ;
-(void)addChangeObserver:(id)arg1 ;
-(id)initWithContainerUuid:(id)arg1 purpose:(id)arg2 inDatabase:(id)arg3 ;
-(void)removeChangeObserver:(id)arg1 ;
-(id)sortItems:(id)arg1 ascending:(char)arg2 ;
-(id)findIndexesForUuids:(id)arg1 ;
-(void)appendEntryForObjectUuid:(id)arg1 ;
-(id)objectUuidsAtIndexes:(id)arg1 ;
-(char)containsObjectUuid:(id)arg1 ;
-(id)convertVersionsToUuids:(id)arg1 ;
-(char)hasCustomOrder;
-(id)customOrderedUuids;
-(unsigned long long)findIndexForVersion:(id)arg1 inSortedVersions:(id)arg2 ascending:(char)arg3 ;
-(void)_setCustomEntries:(id)arg1 ;
-(id)initWithContainerUuid:(id)arg1 purpose:(id)arg2 customEntries:(id)arg3 inDatabase:(id)arg4 ;
-(void)insertEntryForObjectUuid:(id)arg1 atIndex:(unsigned long long)arg2 insertCount:(unsigned long long)arg3 ;
-(void)insertNewEntriesForUuids:(id)arg1 atIndexes:(id)arg2 ;
-(id)removeEntriesAtIndexes:(id)arg1 ;
-(id)containerUuid;
-(id)sortVersions:(id)arg1 ascending:(char)arg2 ;
-(char)loadCustomOrder;
-(unsigned long long)indexOfObjectUuid:(id)arg1 ;
-(void)removeEntryFromIndex:(id)arg1 ;
-(void)addEntryToIndex:(id)arg1 ;
-(unsigned long long)indexOfEntry:(id)arg1 ;
-(void)removeEntry:(id)arg1 ;
-(id)database;
-(id)purpose;
-(void)dealloc;
-(id)description;
-(id)removeEntryAtIndex:(unsigned long long)arg1 ;
@end


/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:08:29 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Library/Application Support/ProApps/Internal Plug-Ins/BrowserKit/iLife.bkplugin/Contents/MacOS/iLife
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <iLife/iLife-Structs.h>
#import <AppKit/NSImageCell.h>

@class NSString, NSView, NSTimer, NSAttributedString, NSImage;

@interface BKILMediaBrowserImageCell : NSImageCell {

	NSString* imagePath;
	NSView* currentControlView;
	NSTimer* durationTimer;
	char mReallyPlaying;
	CGRect _movieBounds;
	NSString* titleString;
	NSAttributedString* attribTitle;
	char drawTitle;
	char mIsClip;
	char mIsProject;
	long long mDisplayDuration;
	long long mImageIndex;
	NSImage* _renderedImage;
	char _renderPending;

}
+(CGSize)_sizeForImage:(id)arg1 withTargetSize:(CGSize)arg2 scrubbing:(char)arg3 ;
+(CGRect)rectForImage:(id)arg1 withBounds:(CGRect)arg2 scrubbing:(char)arg3 ;
+(CGSize)sizeForImage:(id)arg1 withTargetSize:(CGSize)arg2 scrubbing:(char)arg3 ;
+(id)labelFont;
+(id)labelColor;
-(void)setRenderedImage:(id)arg1 ;
-(id)imageBitmapRep:(char)arg1 ;
-(void)normalizeImageSize;
-(CGRect)movieRectForBounds:(CGRect)arg1 size:(CGSize)arg2 ;
-(void)setDrawsTitle:(char)arg1 ;
-(char)drawsTitle;
-(void)setIsClip:(char)arg1 ;
-(char)isClip;
-(void)setIsProject:(char)arg1 ;
-(char)isProject;
-(id)renderedImage;
-(void)setImageIndex:(long long)arg1 ;
-(long long)imageIndex;
-(id)init;
-(void)finalize;
-(void)dealloc;
-(void)setImage:(id)arg1 ;
-(void)setTitle:(id)arg1 ;
-(id)title;
-(void)setRepresentedObject:(id)arg1 ;
-(void)drawInteriorWithFrame:(CGRect)arg1 inView:(id)arg2 ;
-(CGRect)imageRectForBounds:(CGRect)arg1 ;
-(void)_setControlView:(id)arg1 ;
-(char)isPlaying;
-(void)setImagePath:(id)arg1 ;
-(id)imagePath;
@end


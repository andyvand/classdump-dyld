/*
* This header is generated by classdump-dyld 0.7
* on Sunday, February 15, 2015 at 4:08:56 AM Central European Standard Time
* Operating System: Version 10.10.2 (Build 14C109)
* Image Source: /Library/Java/JavaVirtualMachines/jdk1.8.0_25.jdk/Contents/Home/jre/lib/libawt_lwawt.dylib
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <libawt_lwawt.dylib/libawt_lwawt.dylib-Structs.h>
#import <libawt_lwawt.dylib/NSOpenSavePanelDelegate.h>

@class NSString, NSArray;

@interface CFileDialog : NSObject <NSOpenSavePanelDelegate> {

	unsigned char fHasFileFilter;
	jobjectRef fFileDialog;
	long long fPanelResult;
	NSString* fTitle;
	NSString* fDirectory;
	NSString* fFile;
	int fMode;
	char fMultipleMode;
	char fNavigateApps;
	char fChooseDirectories;
	NSArray* fURLs;

}
-(void)safeSaveOrLoad;
-(id)initWithFilter:(unsigned char)arg1 fileDialog:(jobjectRef)arg2 title:(id)arg3 directory:(id)arg4 file:(id)arg5 mode:(int)arg6 multipleMode:(char)arg7 shouldNavigate:(char)arg8 canChooseDirectories:(char)arg9 withEnv:(const JNINativeInterface_*)arg10 ;
-(char)askFilenameFilter:(id)arg1 ;
-(void)disposer;
-(char)userClickedOK;
-(void)dealloc;
-(id)URLs;
-(char)panel:(id)arg1 shouldEnableURL:(id)arg2 ;
@end

